    ...
    <div class="relative container mx-4 py-4 w-[8.5in] h-[11in]">
      <div class="grid grid-cols-3 gap-x-[0.2in] gap-y-[0.1in]">
        @foreach ($chunk as $equipment)
          <div class="pt-8 w-[2.7in] h-[1.5in] bg-white rounded-sm px-3 border border-gray-300 relative"
            style="background-image: url('{{ asset('images/Label BG.png') }}'); background-repeat: no-repeat; background-size: contain; background-position: center center;">
            <div class="relative flex flex-col text-[9px] p-1 text-gray-700">
              <div class="flex">
                <span class="w-[30%] text-gray-800">Client Name</span>
                <span class="w-[10%]">-</span>
                <span class="w-[60%] capitalize"
                  style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis; white-space: normal;">
            ...
            <img src="{{ asset('storage/qrcodes/equipment_' . $equipment['id'] . '.png') }}" alt="Your Image"
              class="absolute bottom-7 right-4 w-9 h-9">
          </div>
        @endforeach
      </div>
