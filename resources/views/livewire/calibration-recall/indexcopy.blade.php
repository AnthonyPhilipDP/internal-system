  ...
  <div
    ...
    <div class="flex justify-between mb-4">
      <div class="flex flex-col gap-1 w-[60%]">
        @if (!empty($customerData['contact_persons']))
          @if ($customer['contact_persons'][0]['identity'] === 'male')
            <p class="text-sm font-semibold text-gray-700 truncate capitalize">Mr.
              {{ $customerData['contact_persons'][0]['name'] }}</p>
          @elseif ($customerData['contact_persons'][0]['identity'] === 'female')
        ...
        <p class="text-sm font-bold text-gray-700 truncate">Client: <span
            class="uppercase">{{ $customerData[0]['name'] }}</span></p>
        @if (!empty($customerData['contact_persons'][0]['contact2']))
          <p class="text-sm font-semibold text-gray-700">
            Fax: {{ $customerData['contact_persons'][0]['contact2'] }}
          </p>
        @endif
        @if (!empty($customerData['contact_persons'][0]['contact1']))
          <p class="text-sm font-semibold text-gray-700">
            Tel: {{ $customerData['contact_persons'][0]['contact1'] }}
          </p>
        @endif
        @if (!empty($customerData['email']))
          <p class="text-sm font-semibold text-gray-700">Email: {{ $customerData['email'] }}</p>
        @endif
      </div>
      <div class="flex flex-col w-[40%] items-end">
        <div class="items-center">
          ...
          <div class="mt-4 text-lg text-center font-bold text-gray-800 border border-red-400 bg-yellow-300 py-4 px-12">
            {{-- DUE: {{ \Carbon\Carbon::parse($customerData['equipment'][0]['calibrationDue'])->format('F Y') }} --}}
            {{ $customerData['contact_persons'][0]['contact2'] }}
          </div>
        </div>
      </div>
    </div>
    ...
    @if ($chunkIndex === 0)
      <p class="text-xs text-gray-800 my-2 text-justify">
        @if (!empty($customerData['contact_persons']))
          @if ($customerData['contact_persons'][0]['identity'] == 'male')
            Dear Sir,
          @else
            Dear Madam,
          @endif
      ...
      <table class="min-w-full divide-y divide-white table-fixed w-full">
        @if ($equipmentChunks->count() > 1)
          <caption class="caption-bottom text-xs text-gray-500 font-mono mt-2">
            Number of equipment in this page: {{ $customerData['equipment']->count() }}
          </caption>
        @endif
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-left">
          @foreach ($customerData['equipment'] as $equipment)
    ...
    <div>
      @if (count($customerData['equipment']) > 1)
        <p class="text-xs mt-2 text-gray-700 text-center">
          Total number of equipments: <span class="font-semibold">{{ count($customerData['equipment']) }}</span>
        </p>
      @else
        <p class="text-xs mt-2 text-gray-700 text-center">
          Total number of equipment: <span class="font-semibold">{{ count($customerData['equipment']) }}</span>
        </p>
      @endif
    </div>
    <!-- Letter Content -->
